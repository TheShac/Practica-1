...
export async function listLibrosByUser(usuario_id) {
  const [rows] = await pool.query(
    `SELECT libro_id, usuario_id, nombre_libro, editorial, lugar, ano,
...
export async function createLibro(usuario_id, data) {
  const {
    nombre_libro, editorial, lugar, ano,
    autor_principal, autores, link_verificacion, estado,
  } = data;

  const [result] = await pool.query(
    `INSERT INTO libro
       (usuario_id, nombre_libro, editorial, lugar, ano,
        autor_principal, autores, link_verificacion, estado)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
...
export async function updateLibro(libro_id, data) {
  ...
  await pool.query(
    `UPDATE libro
     SET nombre_libro = ?, editorial = ?, lugar = ?, ano = ?,
         autor_principal = ?, autores = ?, link_verificacion = ?, estado = ?
     WHERE libro_id = ?`,
    [
      nombre_libro,
