...
export async function listCapLibrosByUser(usuario_id) {
  const [rows] = await pool.query(
    `SELECT cap_id, usuario_id, nombre_capitulo, nombre_libro, editorial,
            lugar, ano, autor_principal, autores, link_verificacion, estado
     FROM cap_libro
     WHERE usuario_id = ?
...
export async function createCapLibro(usuario_id, data) {
  const {
    nombre_capitulo, nombre_libro, editorial, lugar, ano,
    autor_principal, autores, link_verificacion, estado,
  } = data;

  const [result] = await pool.query(
    `INSERT INTO cap_libro
       (usuario_id, nombre_capitulo, nombre_libro, editorial, lugar, ano,
        autor_principal, autores, link_verificacion, estado)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      usuario_id, nombre_capitulo,
      nombre_libro      || null,
      editorial         || null,
      lugar             || null,
      ano               || null,
...
export async function updateCapLibro(cap_id, data) {
  ...
  await pool.query(
    `UPDATE cap_libro
     SET nombre_capitulo = ?, nombre_libro = ?, editorial = ?, lugar = ?, ano = ?,
         autor_principal = ?, autores = ?, link_verificacion = ?, estado = ?
     WHERE cap_id = ?`,
    [
      nombre_capitulo,
