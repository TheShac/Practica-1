...
export async function createTesis(data) {
  const {
    ...
    tesis_dirigida, ano, autor, link_verificacion, rol_guia, nivel_programa,
  } = data;
  ...
  const [result] = await pool.query(
    ...
    [
      usuario_id, titulo_tesis, nombre_programa, institucion,
      tesis_dirigida, ano, autor, link_verificacion || null,
      rol_guia, nivel_programa,
    ]
  );
...
export async function updateTesis(tesis_id, data) {
  const {
    titulo_tesis, nombre_programa, institucion, tesis_dirigida,
    ano, autor, link_verificacion, rol_guia,
  } = data;

  await pool.query(
    `UPDATE tesis
     SET titulo_tesis = ?, nombre_programa = ?, institucion = ?,
         tesis_dirigida = ?, ano = ?, autor = ?,
         link_verificacion = ?, rol_guia = ?
     WHERE tesis_id = ?`,
    [
      titulo_tesis, nombre_programa, institucion, tesis_dirigida,
      ano, autor, link_verificacion || null, rol_guia, tesis_id,
    ]
  );
}

export async function deleteTesis(tesis_id) {
  await pool.query(`DELETE FROM tesis WHERE tesis_id = ?`, [tesis_id]);
}