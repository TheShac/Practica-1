...
export default function FichaAcademicaModal({ show, academico, onClose }) {
  ...
  const nombreCompleto = useMemo(() => {
    if (!academico) return "";
    return `${academico.primer_nombre} ${academico.segundo_nombre || ""} ${academico.primer_apellido} ${academico.segundo_apellido || ""}`;
  }, [academico]);
  ...
  useEffect(() => {
    ...
    async function loadFicha() {
      try {
        setLoading(true);
        const data = await getFichaAcademica(academico.usuario_id);
        setFicha(data);
      } finally {
        setLoading(false);
      }
    ...
    <div className="fa-modal-overlay" onClick={onClose}>
      <div className="fa-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="fa-modal-header">
          <h4 className="fa-modal-title">Ficha Académica: {nombreCompleto}</h4>
            ...
            <Section title="Listado de proyectos de investigación, últimos 5 años">
              {investigaciones.length === 0 ? (
                ...
                investigaciones.map((i, index) => (
                  <div key={index} className="fa-empty" style={{ marginBottom: "15px" }}>
