...
export async function createUserHandler(req, res) {
  try {
    const { rut, primer_nombre, primer_apellido, password, rol, rol_id } =
      req.body;
...
export async function getRolesAcademico(req, res) {
  try {
    res.json(await listRolesAcademico());
  } catch {
    res.status(500).json({ message: "Error obteniendo roles académicos" });
  }
}
...
export async function createRolAcademicoHandler(req, res) {
  try {
    ...
    if (!tipo_academico)
      return res.status(400).json({ message: "tipo_academico es obligatorio" });
    const id = await createRolAcademico(tipo_academico);
    res.status(201).json({ rolaca_id: id, tipo_academico });
  } catch {
    res.status(500).json({ message: "Error creando rol académico" });
  }
}
...
export async function updateRolAcademicoHandler(req, res) {
  try {
    ...
    res.status(500).json({ message: "Error actualizando rol académico" });
  }
}
...
export async function deleteRolAcademicoHandler(req, res) {
  try {
    ...
    res.status(500).json({ message: "Error eliminando rol académico" });
  }
}
