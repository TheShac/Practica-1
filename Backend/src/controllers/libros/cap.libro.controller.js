...
export async function listCapLibrosDeAcademico(req, res) {
  try {
    const { usuarioId } = req.params;
    const rows = await listCapLibrosByUser(usuarioId);
    res.json(rows);
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Error listando capítulos de libro" });
...
export async function createCapLibroParaAcademico(req, res) {
  try {
    const { usuarioId } = req.params;
    const body = req.body || {};
    if (!body.nombre_capitulo)
...
export async function updateCapLibroParaAcademico(req, res) {
  try {
    const { id } = req.params;
    const body = req.body || {};
...
export async function deleteCapLibroParaAcademico(req, res) {
  try {
    const { id } = req.params;
